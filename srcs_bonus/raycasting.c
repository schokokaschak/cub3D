...
/*   Updated: 2023/04/27 11:30:36 by nradin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	calc_draw(t_game *game)
{
	...
	r->draw_start = -r->line_height / 2 + game->screen_height / 2 + r->pitch;
	if (r->draw_start < 0)
		r->draw_start = 0;
	r->draw_end = r->line_height / 2 + game->screen_height / 2 + r->pitch;
...
static void	draw_walls(t_game *game, int i)
{
	...
	rays->texpos = (rays->draw_start - rays->pitch - game->screen_height / 2 + \
...
int	ft_routine(t_game *game)
{
	...
	mlx_hook(game->win, 6, 0, mouse_move_handler, game);
	mlx_hook(game->win, 2, 1L << 0, keys, game);
	floor_ceiling(game);
	wall_casting(game);
	draw_minimap(game, 0, 0);
